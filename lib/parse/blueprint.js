...
module.exports = function(filePath, autoOptions, basePathKey, routeMap) {
    return function(cb) {
        ...
        drafter.parse(data, options, function(err, result) {
            ...
            // get basepath if a key name is set in the configuration options
            if (basePathKey) {
            ...
            // append basepath to url paths if one exists
            ...
            function getBasePath(result, basePathKey) {
                var enclosingSlashes = /^\/|\/$/g;
                return result.ast.metadata.reduce(function (basePath, item) {
                    if (item.name.toLowerCase() === basePathKey.toLowerCase()) {
                        return '/' + item.value.replace(enclosingSlashes, '');
                    }
                    return basePath;
                }, '');
            }
